...
- **Beneficio misurabile dalla libreria di skill, confermato su 5 run completi (aggiornato 2026-08-24)**: il primissimo run (n=6-16 per cella, prima che esistessero i 5 run sotto) non mostrava alcun beneficio — quel dato è rimasto per un po' come unica conclusione riportata qui, ormai superato. I 5 run completi disponibili oggi (`experiment0`-`experiment4`, n per cella da 4 a 39) mostrano invece Small+Librerian (F) battere Small da solo (B) in **tutti e 5**, mai pari o peggio: +7.7%, +4.8%, +11.5%, +25.0%, +5.1%. Aggregato su n=116 confronti: B=88.8%, F=96.6%, **+7.8 punti percentuali**.
  - **Due limiti onesti su questo risultato, non nascosti**: (1) i 5 run condividono in gran parte lo stesso catalogo di task e l'Optimizer ha compresso/riverificato le skill tra un run e l'altro (vedi sezione "Optimizer" in ogni recap) — non sono repliche indipendenti pulite, parte del guadagno potrebbe riflettere la libreria che si adatta a questi task specifici, non generalizzazione pura. (2) Il meccanismo dietro il guadagno non è uniforme: l'indagine causale su `floating_point_equality` (in `experiment0`'s recap) mostra che quella skill applica una sostituzione meccanica di una costante letterale, non una derivazione sensibile alla scala del task — "F batte B" non implica automaticamente ragionamento migliore, in certi casi è recitazione affidabile di una regola.
...
Ricerca in corso, non un prodotto. 5 run reali completi finora (`experiment0`-`experiment4`, log grezzi in `logs/`, versionati in questo stesso repo), non repliche indipendenti pulite (vedi i limiti dichiarati sopra) — i risultati sono riportati con l'incertezza che meritano, non come conclusioni. Vedi i commit e la history per lo sviluppo incrementale (bug trovati e corretti in diretta, non nascosti a posteriori).
