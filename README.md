...
F batte B in tutti e 5 i run completi, mai pari o peggio — aggregato su n=116, +7.8 punti percentuali. Il primissimo run mai fatto su questo progetto (n=6-16 per cella, più piccolo e più vecchio di ognuno di quelli sopra) non mostrava alcun beneficio, ed è rimasto per un po' come unica conclusione scritta in questo README anche dopo che i run sotto lo avevano già smentito — non perché nascosto di proposito, solo perché nessuno aveva ricontrollato. Non l'ho cancellato dalla storia per pulizia, semplicemente non è più la conclusione giusta.

Due limiti veri, non nascosti sotto la tabella:

**Non sono repliche indipendenti pulite.** I run condividono in gran parte lo stesso catalogo di task, e tra un run e l'altro l'Optimizer ha compresso e riverificato le skill esistenti (vedi la sezione Optimizer in ogni `recap.md`). Parte del vantaggio di F potrebbe riflettere la libreria che si affina su questi task specifici, non generalizzazione a task mai visti.

**Il guadagno non è uniforme, meccanismo per meccanismo.** Guardando il run più completo finora (`experiment4`, n=39) sulla componente Δ Retrieval (FULL meno NONE, dentro la stessa config): quella di F è negativa (-3.2%); quella di C è a zero, mai negativa — il meccanismo di retrieval non aiuta allo stesso modo in ogni config, anche quando l'accuracy finale coincide. Il dettaglio task-per-task è in `logs/experiment4/recap.md` e in `logs/experiment4/thesis.html`.

Sul costo: F paga in media tra +116 e +343 token di input in più rispetto a B per usare una skill già pronta (l'"overhead di retrieval", diverso in ogni run a seconda di quali skill vengono recuperate), mentre costruire una skill nuova (l'Optimizer che comprime e riverifica) costa circa 1000-2000 token una tantum per pattern. Ogni `recap.md` calcola un breakeven per pattern (dopo quanti usi il costo di costruzione si ripaga rispetto a chiamare sempre Expert) e una classificazione economica a tre vie — `ECONOMICALLY_POSITIVE` se conviene sui token, `ACCURACY_POSITIVE` se non conviene sui token ma ha risolto qualcosa che A o B non risolvevano, `NEGATIVE` altrimenti (che non significa "butta via la skill": con questi numeri piccoli può avere valore che le due metriche non catturano). Sul run più completo (`experiment4`), di 10 pattern coperti 1 risulta economicamente positivo e 1 accuracy-positivo — gli altri 8 sono `NEGATIVE`, un dato scomodo che è più onesto mostrare che nascondere.
<!-- AUTO:RISULTATI:END -->
...
**Documento unificato**: [`THESIS.md`](../THESIS.md) (nella cartella padre di questo repository) racconta l'intera ricerca — questo progetto e Aria insieme, in linguaggio accessibile a chi non conosce nessuno dei due, con lo schema dati e il percorso di una skill nel codice per entrambi i lati, legenda inclusa. Questa sezione resta la versione tecnica breve, mantenuta qui per chi legge solo questo README.
...
L'idea: l'uso quotidiano reale di Aria può produrre dati di comportamento genuini per la stessa domanda di ricerca, invece di richiedere solo quest sintetiche. Il porting conserva la separazione fra contenuto procedurale, Librarian, budget ed evidenza, ma non genera più skill organiche dagli scambi e non promuove o retrocede il testo scritto dall'utente. Le istruzioni intenzionali funzionano subito; CALL, OUTCOME e BUDGET_DROP distinguono applicazione, esito e scarto di contesto. Il futuro oggetto di ottimizzazione è la politica di applicazione — trigger, fase, combinazioni e costo — con confronti rispetto a un baseline senza skill e senza riscritture silenziose.

**Prima evidenza reale, non solo di principio (2026-09-15).** Un benchmark su un modello Gemma locale ha applicato la stessa metodologia dell'esperimento più completo di questo progetto (coverage FULL/NONE, analisi controfattuale, giudizio automatico invece di un'euristica) al meccanismo di retrieval di Aria ("Archivist") contro il modello nudo: su 22 coppie confrontabili, Archivist 17/22 (77%) contro diretto 13/22 (59%) — un solo run, non conclusivo, ma nella stessa direzione dei risultati qui sopra. Parallelamente, il test causale usato sull'indagine `floating_point_equality` (caso noto + manipolazione diretta) è stato applicato a otto meccanismi di autoverifica scritti nel codice di produzione di Aria (`Jarvis/FOUNDATION_SKILLS_LOG.md`) — e la lezione di quell'indagine ("nominare ogni caso scomodo, non fidarsi che il modello generalizzi da solo un principio") è diventata essa stessa una skill di sistema in Aria. Dettagli in `THESIS.md`.
...
**Un caso in cui il mirroring va fermato al principio, non all'implementazione (osservazione utente del 2026-09-13, verificata contro il codice di Aria).** La condivisione automatica cross-utente delle skill — promozione a "globale" solo dopo verifiche indipendenti da più utenti sopra una soglia che cresce con la base utenti, lo stesso meccanismo di trasferimento di conoscenza tra "generazioni" di questo progetto — probabilmente non si applica ad Aria così com'è. Motivo: le uniche skill davvero generiche e trasferibili che esistono in Aria oggi (le 5 "seed skill", una in più da quando questa nota è stata scritta — vedi `Jarvis/src/lib/skills.ts`) non nascono da distillazione dell'uso di un utente poi verificata da altri — sono scritte a mano dal founder. Le skill che nascono dall'uso reale di UN utente sono tarate su quell'utente (materia, corso, ADHD specifico), non su un pattern comparabile tra persone diverse. Nel dominio Python di questo progetto il trasferimento funziona perché i bug pattern sono oggettivamente comparabili tra istanze diverse (stesso linguaggio, stessa classe di errori); in Aria un "compito comparabile tra agenti" non esiste quasi mai — più fedelmente si porta il meccanismo (soglia che scala con la popolazione, verifica cross-utente), più si espone questo disallineamento. Il contributo reale che l'uso quotidiano di Aria può dare a questa ricerca resta quello descritto sopra — **adattamento persistente di un singolo agente nel tempo** — non il trasferimento di conoscenza tra agenti, che va continuato a testare qui, dove i compiti sono davvero comparabili. Coerente con la clausola già presente in questa nota: "porta il principio, non l'implementazione meccanica, quando le differenze reali di Aria lo richiedono." Il codice di condivisione cross-utente dormiente lato Aria è stato rimosso di conseguenza (`migration_2026-09-13c_remove_organic_skill_promotion.sql`).
