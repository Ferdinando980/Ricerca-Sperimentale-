# Recap -- experiment_verification_2026_09_17b
_Generato: 2026-09-18 00:34 UTC_
...
> 💸 **Analisi controfattuale costo di retrieval** (F vs B stesso task, stesso experiment_id -- zero chiamate nuove): su 6 quest con skill iniettata, **1 (16.7%)** hanno visto B passare comunque senza alcun aiuto -- overhead di token pagato senza beneficio misurato su quella quest specifica (207 token totali). 4 quest mostrano un beneficio reale (B falliva, F ha passato), 1 mostrano lo skill peggiorare l'esito (spesso il sintomo di un difetto nel benchmark, non dello skill -- controlla sopra). Pattern con almeno un beneficio misurato in questo run: floating_point_equality, wrong_string_case_comparison.
...
|---|---|---|---|---|---|
| A | KNOWN | 10 | 90.0% | $0.0000 | 13173 |
| A | NOVEL | 8 | 100.0% | $0.0000 | 9081 |
| A | VARIANT | 21 | 100.0% | $0.0000 | 6493 |
| B | KNOWN | 10 | 80.0% | $0.0000 | 18564 |
| B | NOVEL | 8 | 87.5% | $0.0000 | 16201 |
| B | VARIANT | 21 | 66.7% | $0.0000 | 18666 |
| C | KNOWN | 10 | 90.0% | $0.0000 | 19205 |
| C | NOVEL | 8 | 87.5% | $0.0000 | 16841 |
| C | VARIANT | 21 | 66.7% | $0.0000 | 17643 |
| F | KNOWN | 10 | 90.0% | $0.0000 | 20368 |
| F | NOVEL | 8 | 87.5% | $0.0000 | 16026 |
| F | VARIANT | 21 | 85.7% | $0.0000 | 19185 |

**Totali per config:**
- A -- Expert da solo: accuracy 97.4%, costo totale $0.0000, latenza media 8737ms, n=39
- B -- Small da solo: accuracy 74.4%, costo totale $0.0000, latenza media 18134ms, n=39
- F -- Small + Librarian: accuracy 87.2%, costo totale $0.0000, latenza media 18840ms, n=39
- C -- Small + Cheater (soluzione pregressa): accuracy 76.9%, costo totale $0.0000, latenza media 17879ms, n=39
- B vs A (Small senza aiuto vs Expert): -23.1% accuracy
- F vs B (effetto della Libreria sul modello Small): +12.8% accuracy
- F vs A (quanto F si avvicina all'Expert): -10.3% accuracy
- C vs B (l'accesso a una soluzione pregressa aiuta rispetto a niente?): +2.6% accuracy
- C vs F (soluzione specifica vs skill generalizzata -- la domanda centrale del Cheater Agent): -10.3% accuracy
...
| NONE | 33 | 87.9% |
| FULL | 6 | 83.3% |
- Δ Retrieval (FULL − NONE, dentro F stesso): -4.5%
- Δ Architecture (F-FULL − B): +9.0%
...
- ⚠️ **Δ Retrieval di segno opposto tra F (-4.5%, n=33+6) e C (+2.5%, n=8+10)** -- con questi n puo' essere rumore, ma se si ripete su run future vale la pena chiedersi se il Librarian sta recuperando skill anche quando non aiutano (over-triggering) piu' spesso del Cheater.
...
|---|---|---|---|---|---|---|---|---|
| A | 39 | 5943 | 2801 | 39150 | 47894 | 1228 | 1260 | 97.4% |
| B | 39 | 7542 | 3002 | 0 | 10544 | 270 | 364 | 74.4% |
| F | 39 | 9378 | 3122 | 0 | 12500 | 321 | 368 | 87.2% |
| C | 39 | 10902 | 2997 | 0 | 13899 | 356 | 463 | 76.9% |
...
- Overhead della Libreria (F vs B, stesso task): +47 token di input in media (min 0, max 504) -- il costo di *usare* uno skill gia' pronto.
...
|---|---|---|---|---|---|---|
| `floating_point_equality` | 36945 | 2 | 1621 | 716 | +906 | 40.8 usi |
| `wrong_string_case_comparison` | 6985 | 4 | 2422 | 423 | +1999 | 3.5 usi |
...
- **B** [VARIANT] `mutable_default_argument__flatten_list__variant`
- **B** [NOVEL] `variable_shadowing__reverse_words__novel_b`
- **B** [VARIANT] `wrong_accumulator_init__reverse_words__variant`
...
- **F** [VARIANT] `inverted_boolean_logic__is_valid_username__variant2`
...
- **F** [VARIANT] `wrong_string_case_comparison__count_vowels__variant2`

**Pattern di bug con fallimenti in piu' di un config:**
- `floating_point_equality`: A su floating_point_equality__average_matches_target__known_example, B su floating_point_equality__average_matches_target__known_example, floating_point_equality__temperature_reached__variant, C su floating_point_equality__temperature_reached__variant -- **task diversi della stessa famiglia di bug** falliscono su config diversi. Da controllare quest per quest in `observer_table.csv` (colonna `retrieved_tokens`) se lo skill recuperato e' lo stesso e se l'ha aiutato o confuso.
- `inverted_boolean_logic`: B su inverted_boolean_logic__is_valid_username__variant2, C su inverted_boolean_logic__is_valid_username__variant2, F su inverted_boolean_logic__is_valid_username__variant2 -- stesso task, probabilmente e' il task in se' ad essere difficile per tutti (non e' una domanda sulla Libreria).
- `mutable_default_argument`: B su mutable_default_argument__dedupe_preserve_order__known_example, mutable_default_argument__flatten_list__variant, F su mutable_default_argument__dedupe_preserve_order__known_example, C su mutable_default_argument__flatten_list__variant -- **task diversi della stessa famiglia di bug** falliscono su config diversi. Da controllare quest per quest in `observer_table.csv` (colonna `retrieved_tokens`) se lo skill recuperato e' lo stesso e se l'ha aiutato o confuso.
- `variable_shadowing`: F su variable_shadowing__reverse_words__novel_b, B su variable_shadowing__reverse_words__novel_b, C su variable_shadowing__reverse_words__novel_b -- stesso task, probabilmente e' il task in se' ad essere difficile per tutti (non e' una domanda sulla Libreria).
- `wrong_comparison_operator`: B su wrong_comparison_operator__is_valid_password__variant2, F su wrong_comparison_operator__is_valid_password__variant2, C su wrong_comparison_operator__is_valid_password__variant2 -- stesso task, probabilmente e' il task in se' ad essere difficile per tutti (non e' una domanda sulla Libreria).
- `wrong_string_case_comparison`: B su wrong_string_case_comparison__is_palindrome__variant2, wrong_string_case_comparison__is_valid_username__variant, F su wrong_string_case_comparison__count_vowels__variant2, C su wrong_string_case_comparison__count_vowels__variant2, wrong_string_case_comparison__is_palindrome__variant2, wrong_string_case_comparison__is_valid_username__variant -- **task diversi della stessa famiglia di bug** falliscono su config diversi. Da controllare quest per quest in `observer_table.csv` (colonna `retrieved_tokens`) se lo skill recuperato e' lo stesso e se l'ha aiutato o confuso.

## 7. Uso degli skill (Librarian)
| skill | usi | token medi | quest passate su quest usate |
|---|---|---|---|
| `book_floating_point_equality_v4_v5_v6` | 2 | 526 | 2/2 |
| `book_wrong_string_case_comparison_v2_v3` | 4 | 229 | 3/4 |
